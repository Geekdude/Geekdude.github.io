...
I tried to be clear in the comments of the dotfiles what the configuration lines do, and in this blog post, I will give broader information about the features and reasoning behind the setup.
...
Finally, I also have `s`, which prints the status of the current directory.
...
I have a function that automatically strips training whitespace from files on save except when editing markdown files since trailing whitespace has meaning in markdown.
To help see trailing whitespace in those file formats, I add a $/cdot$ in place of the training space.
